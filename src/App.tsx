    ...
    <div className="min-h-screen bg-[#060910] text-slate-100 flex flex-col font-sans selection:bg-emerald-500/30 selection:text-emerald-200">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-8">
        ...
        <section
          ...
          className="bg-gradient-to-r from-[#09101d] via-[#0c1424] to-[#09101d] border border-emerald-500/30 rounded-2xl p-5 sm:p-6 shadow-2xl relative overflow-hidden"
        >
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
            ...
            <div className="space-y-2">
              ...
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-mono font-extrabold text-white tracking-wide">
                {selectedVuln.name}
              </h1>
            ...
            <div className="shrink-0 flex flex-col sm:flex-row lg:flex-col items-start sm:items-center lg:items-end gap-2">
              <button
                ...
                className="w-full sm:w-auto px-5 py-3.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-black font-mono font-extrabold text-sm sm:text-base flex items-center justify-center gap-3 shadow-[0_0_25px_rgba(16,185,129,0.35)] transition-all hover:scale-[1.02] active:scale-[0.98]"
              >
                <Layers className="w-5 h-5 text-black" />
                <span>SELECT VULNERABILITY TYPE</span>
                <ChevronDown className="w-4 h-4 text-black/80" />
              </button>

              <span className="text-[11px] font-mono text-slate-400 flex items-center gap-1">
                <Sparkles className="w-3 h-3 text-emerald-400" />
                Click to explore <strong>{ALL_VULNERABILITIES.length}+</strong> vulnerability types
              </span>
            </div>
          </div>
        </section>
      ...
      <footer className="border-t border-slate-900 bg-[#04060b] mt-12 py-6 text-center text-xs font-mono text-slate-500">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
            <span>VULN•CALC // Offensive Security &amp; Bug Bounty Toolkit</span>
          </div>
          <div className="flex items-center gap-4 text-[11px] text-slate-400">
            <span>Standard CVSS v3.1 Matrix</span>
            <span>•</span>
